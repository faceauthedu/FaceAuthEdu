...
function buildMetricsSection(people, log) {
  ...
  const checklist = ['Comparación 100% local', 'Sin envío de fotos a terceros', 'Datos en tu navegador']
...
async function saveEnrollment() {
  ...
  try { const person = await Store.add({ name, code: document.getElementById('fCode').value.trim(), career: document.getElementById('fCareer').value.trim(), samples: enrollState.samples }); toast(`Registro de ${person.name} guardado en este navegador.`); location.hash = '#/person/' + person.id; }
